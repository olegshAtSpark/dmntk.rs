...
fn lalr_c_tables() -> String {
  {
    ...
    fs::set_permissions(&script_file_name, Permissions::from_mode(0o755)).expect("setting script permissions failed");
  }
  {
...
#[cfg(test)]
mod tests {
  use crate::generator::{lalr_c_tables, lalr_rust_tables, FEEL_GRAMMAR, TARGET_DIR};
  use std::fs;
  ...
  fn test_feel_grammar() {
    ...
    assert!(len > 1000);
    assert_eq!("%start feel", &FEEL_GRAMMAR[744..755]);
    assert_eq!("%%\n", &FEEL_GRAMMAR[len - 3..]);
  }
